...
weekly scheduler. It has an independent timeout and failure domain while sharing the
daily function's `analytics-writer` lease and BigQuery transaction mutex.
...
def _video_ids_from_latest_snapshot(bq_client: bigquery.Client) -> list[str]:
    ...
    Spends zero Data API quota, which matters given the 2026-08-14 quota-exhaustion incident.
...
def run_refresh(run_date, refresh_run_id: str, log: logging.LoggerAdapter) -> dict:
    ...
    bq_writer = BigQueryWriter(
        ...
        job_labels={"pipeline_run_id": refresh_run_id, "pipeline_writer": "analytics"},
    )
...
def refresh_main(request) -> tuple[dict, int]:
    ...
    try:
        ...
        lease = build_run_lease(
            ...
            domain="analytics-writer", run_id=run_id, entrypoint="refresh_main",
            ttl_seconds=LEASE_TTL_SECONDS,
        )
        with lease:
            result = run_refresh(run_date, run_id, log)
        log.info(f"Analytics refresh handler complete — run_id={run_id}")
