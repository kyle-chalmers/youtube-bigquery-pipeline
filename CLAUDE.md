...
- **`youtube-analytics-refresh`** (trailing-30-day Analytics refresh, live in prod since 2026-09-07): entry point `refresh_main` in `refresh_main.py`; env `GCP_PROJECT`, `BQ_DATASET`, `ANALYTICS_LOOKBACK_DAYS` (must match the daily function's deployed value — 6 as of 2026-09-07), `REFRESH_TRAILING_DAYS` (30), `REFRESH_MIN_ROW_RATIO` (0.5, not yet validated against a real bad day — revisit if it ever fires or misses one). No `YOUTUBE_CHANNEL_ID`/API-key secret binding: video IDs come from the latest `video_metadata` snapshot in BigQuery, not the Data API. Scheduler `youtube-analytics-refresh-weekly`, Sundays 03:00 America/Phoenix, `attemptDeadline=1500s`, `maxRetryAttempts=0` — the zero retry count is load-bearing, see the soft-guard invariant in `analytics_refresh.py`. Deployed by `setup/12_deploy_refresh_function.sh`. First prod run (2026-09-07, manually triggered to validate the deploy rather than waiting for the first Sunday): 533s, 59/60 table-days written, 1 correctly skipped (already-empty day), 0 errors, 0 low-count refusals, 0 duplicate rows.
- **Staging copies:** `youtube-bigquery-pipeline-staging` and `youtube-reporting-ingest-staging`, pointed at `youtube_analytics_staging`, no scheduler. The deploy scripts refuse both directions of a prod/staging cross. The refresh function has no staging copy deployed (rehearsed via `setup/refresh_analytics.py` directly instead, which needs no separate Cloud Function).
- **Runtime:** Python 3.11, 2nd gen, Memory: 512MB. Timeout: 540s (9 min) for the daily and Reporting functions; 1500s (25 min) for the refresh function, sized from a timed staging rehearsal, not guessed — see `analytics_refresh.py`'s module docstring for why a 30-day refresh needs more headroom than a single day's snapshot.
...
- **Trailing-30-day Analytics refresh (`analytics_refresh.py`, `refresh_main.py`):** a weekly, separately-deployed Cloud Function (`youtube-analytics-refresh`) re-fetches and overwrites the trailing ~30 activity days on `daily_video_analytics`/`daily_traffic_sources`, since YouTube revises those numbers for up to 30 days and the legacy Analytics tables otherwise never re-read a day once loaded (the Reporting tables already handle this via `createTime` diffing). Traffic sources use `YouTubeAnalyticsAPI.get_traffic_sources_range` (video+day+trafficSourceType as combined dimensions, one call covers the whole window), NOT the daily cron's per-video-per-day `get_traffic_sources` — the per-day loop measured at ~30 minutes for this channel's 204 videos across 30 days, versus under a second for the range call; do not revert the refresh job to the per-day loop. Writes go through `BigQueryWriter.archive_and_replace`, a staged-then-transactional archive+delete+insert (not the plain two-step `_delete_and_insert`), tagged `load_source='refresh_YYYYMMDD'`; pre-refresh rows land in `daily_video_analytics_refresh_archive`/`daily_traffic_sources_refresh_archive` for a before/after diff. **Soft-guard, not a lock:** this job's window overlaps the daily cron's gap-repair window by design, and both DELETE+INSERT the same partitions; the only thing preventing a collision is scheduling them far apart plus `MAX_RETRY_ATTEMPTS=0` on the refresh job's scheduler entry (a retry would start a second run against a partition the first is still writing to, since Cloud Run doesn't cancel the original invocation just because Scheduler stopped waiting on it). Changing either schedule needs to preserve that separation — full invariant documented at the top of `analytics_refresh.py`; a violation shows up as duplicate `(activity_date, video_id)` rows.
...
- **Cloud Functions:** `youtube-bigquery-pipeline` (refactor promoted 2026-09-06, `ANALYTICS_LOOKBACK_DAYS=6`), `youtube-reporting-ingest` (promoted 2026-09-06, `REPORTING_ENABLED=true`, scheduler `youtube-reporting-daily` at 08:00 and 14:00 Phoenix), and `youtube-analytics-refresh` (promoted 2026-09-07, scheduler `youtube-analytics-refresh-weekly` Sundays 03:00 Phoenix), all 2nd gen, Python 3.11, 512MB. Timeout 540s for the first two, 1500s for the refresh function. Five alert policies exist for production (including `youtube-refresh-failure`); the `-staging` copies of the first two functions' policies stay until the staging soak is closed — the refresh function has no staging deploy or staging alert policy, since it was rehearsed via `setup/refresh_analytics.py` instead.
- **Cloud Scheduler:** daily pipeline runs at 00:10 Phoenix time (`America/Phoenix`, no DST), ten minutes after the Data API quota resets; the Analytics refresh runs weekly, Sundays 03:00 Phoenix, deliberately far from the daily run (see the soft-guard invariant in `cloud_function/analytics_refresh.py`)
